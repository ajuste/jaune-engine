...
(function() {
  ...
  FSFileSystem.prototype = {
    ...
    writeFile : function(path, options, data, cb) {
      ...
      Fs.writeFile(convertPathToAbsolute(path), data, function(err) {
        cb(err);
      });
    },
    /**
    ...
    readFile : function(path, cb, opts) {
      ...
      if (opts.encoding === "binary") {
        this.exists(path, function(exists) {
          cb(!exists ? new Error("file does not exists)") : null, exists ? Fs.createReadStream(absolute) : null);
        });
      }
      else {
        Fs.readFile(absolute, opts.encoding, function(err, file) {
          cb(err, file);
        });
      }
    },
    ...
    exists : function(path, cb) {
      Fs.exists(convertPathToAbsolute(path), function(exists) {
        cb(exists);
      });
    },
    /**
