...
(function() {
  ...
  var DbManager        = require("./db-manager").DatabaseManager;
  ...
  SqlUtil.prototype = {
    ...
    executeChained : function(fn, cb, chainedClient, options) {
      ...
      this.chain(_.bind(onChain, this), chainedClient);
    },
    /**
    ...
    chain : function(cb, chainedClient, options) {
      ...
      var chainedCallback = { connect : onChain };
      /**
      * On ready callback.
      *
      * @callback
      ...
      */
      ...
      this.__getSql().chain(chainedCallback, chainedClient);
    },
    /**
  ...
  Executor.prototype = {
    ...
    doSuccess : function(successCallback, result) {
       ...
       */
      function onCommit() {
        this.doFinally(_.bind(onFinally, this));
      }
      this.client.commit(_.bind(onCommit, this));
    },
    /**
    ...
    doRollback : function(rollbackCallback, result) {
      ...
      this.client.rollback(_.bind(onRollback, this));
    },
    /**
