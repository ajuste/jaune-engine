 ...
 (function() {
  ...
  PostgresClient.prototype = {
    ...
    chain : function(cb, chainedClient) {
      ...
      if (handleConnection) {
        this.connect(_.bind(onConnect, this));
      }
      else {
    ...
    connect : function(cb) {
      Pg.connect(this.opts.server, _.bind(function(err, cli, done) {
        ...
        if (connection.links.length !== 0) {
          cb(undefined, new Error("this connection is dirty - links already present: " + connection.links.length));
        }
        else {
          if (!err) {
          ...
          cb(connection, standarizeError(err));
        }
      }, this));
    },
    /**
  ...
  PostgresConnection.prototype = {
    ...
    begin : function(cb) {
      if (!this.isTransactionOpen()) {
        this.query({ sql: "BEGIN;" }, _.bind(this.__onBeginQueryExecuted, this, cb));
      }
      else {
        onBegin.call(this);
      }
    ...
    rollback : function(cb) {
      if (this.current().handleConnection === true && this.isTransactionOpen()) {
        this.query({ sql: "ROLLBACK;" }, _.bind(this.__onRollbackQueryExecuted, this, cb));
      }
      else {
        cb();
      }
    ...
    commit : function(cb) {
      if (this.current().handleConnection === true && this.isTransactionOpen()) {
        this.query({ sql: "COMMIT;" }, _.bind(this.__onCommitQueryExecuted, this, cb));
      }
      else {
        cb();
      }
    ...
    rollbackAndClose : function(cb) {
      this.rollback(_.bind(this.__onRollbackAndClose, this, cb));
    },
    /**
    ...
    commitAndClose : function(cb) {
      this.commit(_.bind(this.__onCommitAndClose, this, cb));
    },
    /**
    ...
    query : function(query, cb) {
      ...
      this.client.query(query.sql, query.args || [], _.bind(this.__onQueryCallback, this, cb));
    },
    /**
    ...
    call : function(query, cb) {
      ...
      this.query({ sql : "SELECT * FROM " + query.sql + "(" + args.select(function(e) { return "$" + parameterIndex++; }).join(", ") + ");", args : query.args }, cb);
    },
    /**
     ...
     */
    close : function(cb) {
      this.commit(_.bind(this.__onCloseCommit, this, cb));
    },
    /**
